...
const App = () => {
  ...
  const handleConfirm = () => {
    if (showAuthInput) {
      ...
      axios.post('http://j10e101.p.ssafy.io:8000/api/user/check', verificationInfo)
        .then(response => {
          // 성공적으로 응답을 받았을 때
      ...
      // 첫 번째 단계의 조건 검증 및 인증번호 요청은 checkAndShowAuthInput 함수에서 처리
  ...
  const checkAndShowAuthInput = () => {
    ...
    if (PhoneNum.length === 11 && RNum.length === 6 && RNum2.length === 1 && Name.length > 0) {
      ...
      axios.post('http://j10e101.p.ssafy.io:8000/api/user/send', userInfo)
        .then(response => {
          // 성공적으로 응답을 받았을 때
  ...
  const MiddleView3 = () => (
    <View
      ...
      <TextInput
        ...
        returnKeyType="next"
        onSubmitEditing={() => authNumInputRef.current.focus()}
      ></TextInput>
    </View>
  );
